...
worker=(root/"MediaToolWorker.kt").read_text(encoding="utf-8")
...
require(tools,["AtmacaToolPage.PERSON_CROP","AtmacaToolPage.PACKAGER","AtmacaToolPage.VIDEO_FRAMES","ActivityResultContracts.OpenDocumentTree()","collectToolUrisFromTree(","filterToolUris(context, uris, tool)","toolPickerMimeTypes(tool).toTypedArray()","enqueueMediaToolWork(context, tool, selectedUris, option)"],"tool UI")
...
require(worker,["class MediaToolWorker","CoroutineWorker","OneTimeWorkRequestBuilder<MediaToolWorker>()","engine.smartPersonCrop","engine.packageMedia","engine.extractVideoFrames","setProgressAsync(","setForeground(foregroundInfo(tool, 0, 0))","FOREGROUND_SERVICE_TYPE_DATA_SYNC"],"MediaToolWorker")
